...
#
# 4. Risk/Reward = TP_Distance / SL_Distance ≥ min_risk_reward_ratio (config'den)
#    → Pozitif beklenen değer için minimum eşik
#
# [SORUN 7 DÜZELTMESİ]
# calculate_take_profit ve check_risk_reward fonksiyonları artık
# hard-code 1.5 yerine self.risk_cfg.min_risk_reward_ratio kullanıyor.
# settings.yaml'daki değeri değiştirmek anında etkili oluyor.
# =============================================================================
...
class StopLossResult:
    ...
    distance: float        # Entry'den SL'ye mesafe ($)
    distance_pct: float    # Entry'den SL'ye mesafe (%)
...
class TakeProfitResult:
    ...
    distance: float        # Entry'den TP'ye mesafe ($)
    distance_pct: float    # Entry'den TP'ye mesafe (%)
    risk_reward: float     # Fiilen uygulanan RR oranı
...
class PositionSizeResult:
    ...
    risk_amount: float     # Risklenen tutar ($)
    margin_required: float # Gerekli margin ($)
