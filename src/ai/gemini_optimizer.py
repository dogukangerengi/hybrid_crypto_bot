...
class GeminiOptimizer:
    ...
    # =========================================================================
    def _log_interaction(self, symbol, prompt, response):
        """Prompt ve cevabı dosyaya kaydeder."""
        try:
            # logs klasörü yoksa oluştur
            log_dir = Path("logs/gemini_history")
            log_dir.mkdir(parents=True, exist_ok=True)
            
            # Dosya adı: gemini_log_2024-02-12.txt gibi tarihli olsun
            filename = log_dir / f"gemini_log_{datetime.now().strftime('%Y-%m-%d')}.txt"
            
            with open(filename, "a", encoding="utf-8") as f:
                f.write("\n" + "="*50 + "\n")
                f.write(f"ZAMAN: {datetime.now().strftime('%H:%M:%S')}\n")
                f.write(f"COIN: {symbol}\n")
                f.write("-" * 20 + " GÖNDERİLEN PROMPT " + "-" * 20 + "\n")
                f.write(prompt + "\n")
                f.write("-" * 20 + " GELEN CEVAP " + "-" * 20 + "\n")
    ...
    def get_decision(
        ...
        try:
            ...
            self._log_interaction(analysis_input.symbol, prompt, raw_response)
    ...
    def _check_gate(self, ic_score: float) -> GateAction:
        ...
        - < 35  → NO_TRADE: İstatistiksel sinyal yetersiz
        - = 50 → REPORT_ONLY: Sinyal var ama güçlü değil
        - ≥ 50  → FULL_TRADE: Güçlü sinyal, emir gönderilebilir
        """
...
4. Fırsatları kaçırmamak için agresif ol. Ufak riskleri kabul et ve işlem onayı ver.
