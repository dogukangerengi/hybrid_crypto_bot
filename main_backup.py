...
class HybridTradingPipeline:
    ...
    async def run_cycle(self):
        """
        Tek bir tarama-analiz-işlem döngüsünü çalıştırır.
        """
        # --- BURAYI EKLE ---
        if self.notifier:
            try:
                await self.notifier.send_message(f"🔔 Döngü Başladı (#{self._cycle_count}) - Bakiye: ${self.paper_trader.balance:.2f}")
            except Exception as e:
                logger.error(f"Telegram başlangıç mesajı hatası: {e}")
        # -------------------

        cycle_start_time = time.time()
        # ... kodun geri kalanı devam ediyor ...
        self._cycle_count += 1
...
def main():
    ...
    if args.symbol:
        ...
        if not pipeline._init_balance():
            ...
            sys.exit(1)
            # ---------------------------------------------------------------------
        # TELEGRAM BİLDİRİMİ (Burası Eksikti, Ekliyoruz)
        # ---------------------------------------------------------------------
        if self.notifier and cfg.telegram.enabled:
            # Sadece ÖNEMLİ durumlarda mesaj at (İşlem varsa veya Hata varsa)
            # Eğer her döngüde mesaj istersen "or True" ekleyebilirsin.
            should_notify = True (
                report.total_traded > 0 or           # İşlem açıldıysa
                report.status == CycleStatus.ERROR or # Hata varsa
                report.status == CycleStatus.KILLED or # Kill switch aktifse
                True  # <--- BURAYA "or True"EKLE (Geçici olarak her döngüde mesaj atar)
            )
            
            # İstersen her saat başı rapor atması için şunu da ekleyebilirsin:
            # or (self._cycle_count % 1 == 0)  <-- Her döngüde mesaj atar

            if should_notify:
                msg = f"""
📊 *DÖNGÜ RAPORU (#{self._cycle_count})*
⏱️ Süre: {report.elapsed:.1f}s
🔍 Taranan: {report.total_scanned}
✅ İşlem: {report.total_traded}
💰 Bakiye: ${report.paper_balance:.2f}
"""
                # Eğer işlem varsa detaylarını ekle
                if report.total_traded > 0:
                    msg += "\n🔵 *YENİ İŞLEMLER:*\n"
                    for coin in report.coins:
                        if coin.status == "executed":
                            msg += f"• {coin.coin}: {coin.ic_direction} (Güven: {coin.ic_confidence:.0f})\n"

                try:
                    # Asenkron fonksiyonu senkron içinde çalıştır
